...
╰───「 *🥳ʟᴇᴠᴇʟ ᴜᴘ🥳* 」`)
          ...
          sendButton(from, lvlup, '*Kyuraz Bot*', but)
...
${gaya2} ${prefix}contact _62x|Name_
...
Please Choose Whether Document, Audio or Video Below`
        sendButMessage(from, result, `*Kyuraz Bot*`, [
          {
            buttonId: `${prefix}fire ${teks}`,
            buttonText: {
              displayText: ` Document/Other`,
            },
            type: 1,
          },
          {
...
sendButVideo(from, pll, `*Kyuraz Bot*`, get_result, [      
{
buttonId: `${prefix+command}`,
buttonText: {
displayText: `NEXT ➡️`,
...
//----> 1 TEXT <----//
                ...
                case 'freefire':          
                    if (args.length == 0) return reply(`Example: ${prefix + command} Kyuraz`)
                    ini_txt = args.join(" ")
...
case 'memegenerator': case 'memegen':{
 	...
 	case 'stickermeme': case 'memesticker': case 'memestick': case 'stickmeme': case 'stcmeme': case 'smeme':{
						if (args.length < 1) return reply(`Send orders *${prefix + command}* DogeBot`)
									if (q.includes('|')) return reply(`Reply to an image with a caption, For Example *${prefix + command}* Kyuraz`)
									try {
										if (!isQuotedImage) return reply(`Reply to an image!`)
										reply(mess.wait)
										var teks2 = args.join(' ')
...
//══════════[ GAME FEATURES ]══════════//
    ...
    "say i love the bot owner Ryutenzo through voice note",
    ...
    "do you love the bot creator, Ryutenzo?ðŸ¦„",
...
case 'bc':
             ...
             for (let _ of anu100) {
             DogeXeonOP.sendMessage(_.jid, bc100, image, {quoted: fgi, caption: `*「 Kyuraz Bot Broadcast 」*\n\n${body.slice(4)}`})
}
             reply('Broadcast success')
             } else {
             for (let _ of anu100) {
             DogeXeonOP.sendMessage(_.jid, 
			{"contentText": `*「 KYURAZ BOT BROADCASTING MESSAGE 」*\n\n${body.slice(4)}`,
			"footerText": `${tanggal}`,
			"buttons": [
...
DogeXeonOP.sendMessage(_.jid, bcgc, image, { caption: `*「 KYURAZ BOT BROADCAST 」*\n*Group* : ${groupName}\n\n${body.slice(6)}` })
}
reply('')
} else {
for (let _ of groupMembers) {
sendMess(_.jid, `*「 KYURAZ BOT BROADCAST 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`)
}
...
reply(`${err}`)
	...
	DogeXeonOP.sendMessage(`${owner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Ryutenzo",body:"Dont forget to subscribe Xeon",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/dogepic1.jpg'),sourceUrl:"https://wa.me/6283819082780"}}})
        }
	// console.log(e)
	}
}
